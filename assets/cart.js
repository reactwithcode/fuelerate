...
class CartItems extends window.StandardEvents.createViewEventElement(HTMLElement) {
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        CartPerformance.measure(`${eventTarget}:paint-updated-sections`, () => {
          ...
          paintPromise = this.paintSections(parsedState.sections || {}, sectionsToRender).then(() => {
  ...
  // /cart/change.js's `sections` param only resolves sections that are
  // already reachable via `{% section %}` on the current page — e.g.
  // cart-drawer is rendered as a plain snippet, so Shopify can't resolve
  // it there and comes back with `sections: null`. Fall back to the
  // generic Section Rendering API (`?sections=`), which renders any
  // section file by handle regardless of where/how it's included.
  paintSections(sections, sectionsToRender) {
    const apply = (allSections) => {
      sectionsToRender.forEach((section) => {
        const html = this.getSectionInnerHTML(allSections[section.section], section.selector);
        if (html === null) return;
    ...
    const missingSections = sectionsToRender.filter((section) => !sections[section.section]);
    if (missingSections.length === 0) {
      apply(sections);
      return Promise.resolve();
    }

    const missingIds = missingSections.map((section) => section.section);
